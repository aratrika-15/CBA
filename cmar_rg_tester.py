"""
Just a copied implementation of the CBA RG with some changes to test the CMAR classifier
"""
    ...
    def find_prune_rule(this_rule):
        ...
        if len(this_rule_cond_set) >= 2:
            for attribute in this_rule_cond_set:
                ...
                temp_rule = cmar_rule.CMAR_Rule(temp_cond_set, this_rule.class_label, dataset)
...
def join(item1, item2, dataset):
    ...
    new_ruleitem = cmar_rule.CMAR_Rule(new_cond_set, item1.class_label, dataset)
    return new_ruleitem
    ...
    for column in range(0, len(dataset[0])-1):
        ...
        for value in distinct_value:
            ...
            for classes in class_label:
                rule_item = cmar_rule.CMAR_Rule(cond_set, classes, dataset)
